"""
Print statistics for unique bead positions (x, y).

Use this when your protein profile has multiple proteins with the same activation codes,
causing duplicate entries for the same physical bead.
"""

def print_unique_bead_statistics(results_df):
    """
    Print statistics based on unique (x, y) coordinates.

    This removes duplicates caused by multiple proteins having the same activation pattern.
    """
    print("="*60)
    print("UNIQUE BEAD STATISTICS (deduplicated by x, y)")
    print("="*60)

    # Remove duplicates by x,y coordinates, keeping first occurrence
    ...
    if num_duplicates > 0:
        print(f"\nRemoved {num_duplicates} duplicate entries (same x,y with different protein names)")

    # Overall counts for unique positions
...
# Usage in Colab cell:
# unique_beads = print_unique_bead_statistics(results_df)
