...
class TestFileManagerVM:
    ...
    def test_set_dataset_cycle_assignments_allows_unassigned_files(
        ...
        is_valid = vm.set_dataset_cycle_assignments({0: ref_item, 1: cy1_item})
    ...
    def test_load_folder_with_invalid_tiff_file(
        self, mock_file_manager_vm, tmp_path, mocker
    ):
    ...
    def test_delete_files_removes_from_emitted(
        self, mock_file_manager_vm, mock_file_item
    ):
    ...
    def test_apply_shading(self, mock_file_manager_vm, mock_file_item, mocker):
        ...
        with (
            patch("viewmodel.file_manager_vm.load_image", return_value=mock_image),
    ...
    def test_align_channels_no_reference(
        self, mock_file_manager_vm, mock_file_items, mocker
    ):
    ...
    def test_alignment_complete_updates_two_cycles_and_protein(
        ...
        with (
            patch.object(
                vm,
                "_get_brightfield_image",
                return_value=np.zeros((32, 32), dtype=np.uint16),
            ),
            patch(
                "viewmodel.file_manager_vm.AlignmentPreviewDialog", return_value=dialog
            ) as mock_dialog,
        ):
            vm._on_alignment_complete(
                aligned_images, [cy1_item, cy2_item, protein_item]
            )
    ...
    def test_export_task_exports_aligned_two_cycles_and_protein(self, tmp_path):
        export_folder = tmp_path / "export"
        export_folder.mkdir()
    ...
    def test_apply_metadata_shape_uses_min_when_max_size_larger_than_original(
        self, mock_file_manager_vm, mock_file_item
    ):
    ...
    def test_apply_metadata_shape_updates_to_max_size_when_smaller(
        self, mock_file_manager_vm, mock_file_item
    ):
        ...
        expected_shape = (
            original_shape[0],
            min(original_height, new_max_size),
            min(original_width, new_max_size),
        )
        assert vm.files[item.path].shape == expected_shape

    def test_apply_metadata_emits_corrected_signal_when_max_size_too_large(
        self, mock_file_manager_vm, mock_file_item, signal_recorder
    ):
    ...
    def test_apply_metadata_no_corrected_signal_when_max_size_valid(
        self, mock_file_manager_vm, mock_file_item, signal_recorder
    ):
    ...
    def test_load_folder_duplicate_detection(
        self, mock_file_manager_vm, tmp_tiff_folder
    ):
        ...
        with (
            patch("viewmodel.file_manager_vm.list_tiff_files") as mock_list_files,
            patch("viewmodel.file_manager_vm.get_tif_info") as mock_info,
        ):
    ...
    def test_recompute_ensemble_sweep_updates_file_item(
        self, mock_file_manager_vm, mock_file_item
    ):
        vm = mock_file_manager_vm
        vm.files[mock_file_item.path] = mock_file_item
        ...
        with patch(
            "image_processing.compute_ensemble_sweep_stats", return_value=expected_sweep
        ):
    ...
    def test_apply_ensemble_ratio_updates_beads(
        self, mock_file_manager_vm, mock_file_item
    ):
        vm = mock_file_manager_vm
        vm.files[mock_file_item.path] = mock_file_item
        ...
        with patch(
            "image_processing.build_ensembled_beads_from_cache", return_value=new_beads
        ):
    ...
    def test_remove_ensemble_applied_changes_reverts_to_pre_ensemble(
        self, mock_file_manager_vm, mock_file_item
    ):
        vm = mock_file_manager_vm
        vm.files[mock_file_item.path] = mock_file_item
    ...
    def test_bead_generation_task_uses_time_calibrated_progress(
        ...
        with (
            patch(
                "viewmodel.tasks.load_and_constrain_image",
                return_value=np.zeros((2, 32, 32), dtype=np.uint16),
            ),
        ...
        assert emitted
        pre_values = [
            v for v, m in emitted if "Preprocessing brightfield image..." in m
        ]
        assert pre_values
    ...
    def test_bead_generation_task_stardist_sets_workload_before_processing(
        ...
        def capturing_set_workload(self, total_channels, max_size_pixels):
            set_workload_calls.append(
                {"total_channels": total_channels, "max_size_pixels": max_size_pixels}
            )
        ...
        with (
            patch.object(BeadEtaEstimator, "set_workload", capturing_set_workload),
            patch(
                "viewmodel.tasks.load_and_constrain_image",
    ...
    def test_bead_generation_task_heartbeat_emits_between_sparse_callbacks(
        self, qapp, tmp_tiff_path
    ):
